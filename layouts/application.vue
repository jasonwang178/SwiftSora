...
await callOnce(async () => {
  const { data: videos } = await useFetch<Video[]>('/api/videos', { method: 'GET', lazy: true, server: true })

  // @ts-ignore
  communityVideos.value = videos
})
...
onMounted(async () => {
  ...
  const toggleSidebar = (sidebarEl, expand, setExpanded = false) => {
    ...
    if (expand) {
      ...
      bottomMenuEl.classList.add('flex', 'flex-col')
      bottomMenuEl.classList.remove('hidden')
      ...
      bottomMenuEl.classList.remove('flex', 'flex-col')
